...
local CMD = {}
CMD.update = function(param)
    local ok, err = gDb[param.coll_name]:safe_update({_id = param._id}, {['$set'] = param.chgs}, true)
	assert(ok, err)
    return {ok = ok, err = err}
end

CMD.find = function(param)
	local cursor = gDb[param.coll_name]:find(param.filter, param.projection)
    if not cursor or not cursor:hasNext() then
        return {}
    end
    return cursor:next()
end
...
skynet.start(function()
    local c = _create_client()
	gDb = c[db_name]
    skynet.dispatch("lua", function(session, source, cmd, param)
	    local f = assert(CMD[cmd])
        skynet.ret(skynet.pack(f(param)))
	end)
end)
